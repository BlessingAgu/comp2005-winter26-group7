...
/**
 * BlokusGame - Core game that works with or without optional features
 * @author Wellington
 */
public class BlokusGame extends JFrame implements ActionListener
{
	...
	public BlokusGame(int boardSize, String[] playerNames)
	{
		...
		// OPTIONAL: Register with features if they exist
		try {
			Class.forName("gui.GameUI");
			GameUI.setGameWindow(this);
		} catch (ClassNotFoundException e) {
			// GameUI not present - that's OK, game still works
		}
	...
	private void setupGUI()
	{
		...
		// OPTIONAL FEATURE: Options button (only if OptionsMenu exists)
		try {
			Class.forName("gui.OptionsMenu");
			JButton optionsButton = new JButton("Options");
			optionsButton.addActionListener(e -> new OptionsMenu());
			topPanel.add(optionsButton);
		} catch (ClassNotFoundException e) {
			// OptionsMenu not present - that's OK
		}
	...
	// OPTIONAL FEATURE: Called by GameUI if color palettes are enabled
		...
		for (int[] sq : shape)
		{
			...
			if (r >= 0 && r < boardSize && c >= 0 && c < boardSize)
			{
				if (valid)
				{
					...
					// Use color palette if available, otherwise use simple light colors
	...
	// Gets preview color - uses BoardSquareColors if available, fallback if not
	private Color getPreviewColor(int player)
	{
		try {
			...
			// BoardSquareColors not present - use simple fallback
			switch (player)
			{
